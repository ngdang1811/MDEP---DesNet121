    ...
    def forward(self, x):
        if self.warmup:
            ...
            if getattr(self, 'freeze_mask', False) and hasattr(self, 'cached_mask'):
                effective_weight = self.weight * self.cached_mask
            else:
                ...
                self.cached_mask = differentiable_mask.detach()
    ...
    for name, module in model.named_modules():
        if isinstance(module, (MDEPLinear, MDEPConv2d)):
            ...
            if not hasattr(module, 'grad_L_S'):
                scores_grad = module.scores.grad
                module.grad_L_S = scores_grad.clone().detach() if scores_grad is not None else torch.zeros_like(module.scores)
            ...
            # g2: per-weight loss gradient magnitude = |∂L_EFL/∂S_ij|
            g2 = torch.abs(module.grad_L_S)
            g2_min = g2.min().item()
            g2_max = g2.max().item()
            g2_norm = (g2 - g2_min) / (g2_max - g2_min + 1e-8)

            # Tiềm năng mọc lại (Growth Potential)
            # :math: G_{ij}^{(t)} = \text{Norm}\left( u_{e,i}^{(node)} \right) \times \text{Norm}\left( \left| \frac{\partial \mathcal{L}_{EFL}}{\partial S_{ij}} \right| \right)
...
class ISICDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Try 1: Load from HDF5 archive (Fastest)
        ...
        # Try 2: Load from individual image file (Fallback)
        if image is None and self.image_dir:
            img_path = os.path.join(self.image_dir, f"{isic_id}.jpg")
            try:
                image = Image.open(img_path).convert('RGB')
            except Exception:
                image = Image.new('RGB', (224, 224), color='black') # Final Fallback
...
def get_isic_dataloaders(batch_size=32, test_ratio=0.2):
    ...
    # VŨ KHÍ TỐI THƯỢNG CHỐNG TRÀN RAM: Tắt hoàn toàn đa tiến trình (num_workers=0) để chống memory leak từ HDF5
...
def print_sparsity_report(model):
    ...
    print("-" * 75)
    print(r"  :math: \text{MACs}_{sparse} = \frac{1}{2} \sum_{l=1}^L \left( C_{in}^{(l)} \times C_{out}^{(l)} \times K^2 \times H \times W \right) \approx 50\% \text{ MACs}_{dense}")
...
def evaluate(model, test_loader, device, num_classes):
    ...
    if num_classes == 2:
        ...
        ppv = tp / (tp + fp + 1e-8)
        npv = tn / (tn + fn + 1e-8)
        f2_score = (5 * ppv * sensitivity) / (4 * ppv + sensitivity + 1e-8)
        ...
        ppv_opt = tp_opt / (tp_opt + fp_opt + 1e-8)
        npv_opt = tn_opt / (tn_opt + fn_opt + 1e-8)
        f2_score_opt = (5 * ppv_opt * sensitivity_opt) / (4 * ppv_opt + sensitivity_opt + 1e-8)
    else:
    ...
    if num_classes == 2:
        ...
        print(f"  PPV (Precision)       : {ppv_opt:.4f}")
...
def main():
    ...
    # ── Model: DenseNet-121 with EDL head ──────────────────────────
    ...
    checkpoints = sorted(glob.glob(os.path.join(checkpoint_dir, 'checkpoint_epoch_*.pth')), key=os.path.getmtime)
    if checkpoints:
        ...
        # --- Hành động 2: Cập nhật hàm Auto-Resume ---
        # Xử lý an toàn: Xóa chữ 'module.' nếu checkpoint cũ được lưu bằng DataParallel
    ...
    max_session_hours = 11.5  # Ngắt an toàn trước mốc 12h của Kaggle
    target_stop_epoch = 10    # Chủ động ngắt ở Epoch 10 (Sửa tham số này tùy ý)
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        # --- Hành động 3: Cập nhật hàm Lưu Checkpoint ---
        ...
        # Quản lý dung lượng: Chỉ giữ lại 2 file checkpoint gần nhất
        checkpoints_to_keep = sorted(glob.glob(os.path.join(checkpoint_dir, 'checkpoint_epoch_*.pth')), key=os.path.getmtime)
        while len(checkpoints_to_keep) > 2:
            oldest_ckpt = checkpoints_to_keep.pop(0)
            os.remove(oldest_ckpt)
            print(f"  Removed old checkpoint: {oldest_ckpt} to save disk space.")
            
        # --- KIỂM TRA ĐIỀU KIỆN HẠ CÁNH MỀM ---
        elapsed_hours = (time.time() - session_start_time) / 3600.0
        
        if (epoch + 1) >= target_stop_epoch:
            print(f"\n🛑 Đã đạt mốc Epoch {target_stop_epoch} theo kế hoạch. Chủ động ngắt phiên để chia nhỏ tiến trình!")
            break
