...
def update_scores_agents(model, beta=1.0):
    ...
    for module in model.modules():
        if isinstance(module, (MDEPLinear, MDEPConv2d)):
            # Sửa lỗi không gian tối ưu: Kiểm tra module.scores.grad thay vì module.grad_L_w
            if module.scores.grad is None and getattr(module, 'grad_L_S', None) is None:
                continue
            ...
            # --- Feature Flow ---
            # :math: h^{(l)} = \phi \left( \tilde{W}^{(l)} [h^{(0)}, \dots, h^{(l-1)}] + b^{(l)} \right)
            ...
            # (Giải thích: Nhờ Concatenation, khoảng cách 1 bước bảo toàn full rank ma trận Jacobian cho Astrocyte).
