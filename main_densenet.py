...
class ISICDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if getattr(self, 'hdf5_path', None) and globals().get('HAS_H5PY', False):
            try:
                hf = getattr(self, '_get_hdf5', lambda: None)()
        ...
        if image is None and getattr(self, 'image_dir', None):
...
def main():
    ...
    # 1. Load off-the-shelf DenseNet from torchvision
    # DenseNet121 is used as the backbone
    ...
    for epoch in range(total_epochs):
        ...
        # Trích xuất state_dict an toàn từ DataParallel để tránh lỗi AttributeError khi load
        model_state = model.module.state_dict() if isinstance(model, nn.DataParallel) else model.state_dict()
        
        # Lưu file checkpoint với tên chứa số epoch
